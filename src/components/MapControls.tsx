...
import { clampZoom } from '@/lib/map-page-zoom';
...
export default function MapControls({ mapRef, onInteract }: MapControlsProps) {
  ...
  const step = useCallback((move: Move) => {
    ...
    if (move.kind === 'zoom') {
      map.easeTo({
        zoom: clampZoom(map.getZoom() + move.dir * ZOOM_STEP, map.getMinZoom(), map.getMaxZoom()),
        duration: STEP_MS,
      });
    }
  ...
  const press = useCallback((move: Move) => {
    ...
    const leg = () => {
      ...
      if (move.kind === 'zoom') {
        m.easeTo({
          zoom: clampZoom(m.getZoom() + move.dir * ZOOM_RATE * (LEG_MS / 1000), m.getMinZoom(), m.getMaxZoom()),
          duration: LEG_MS,
          easing: linear,
        });
      } else {
