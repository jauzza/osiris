...
import { X, ExternalLink, RefreshCw, MapPin, Camera, CameraOff, Maximize2, PlayCircle, LayoutGrid } from 'lucide-react';
import Hls from 'hls.js';
import { isHostedOffPlatform, liveFeedAtSource, localEmbed, needsResolution, offPlatformView } from '@/lib/camera-feed';
import { refreshInterval } from '@/lib/camera-preview';

interface CameraViewerProps {
  camera: any | null;
  onClose: () => void;
  onLocate?: (lat: number, lng: number) => void;
  onAddToGrid?: (camera: any) => void;
}

export default function CameraViewer({ camera, onClose, onLocate, onAddToGrid }: CameraViewerProps) {
  ...
  useEffect(() => {
    ...
    const every = refreshInterval('jpg', camera) || 5000;
    const iv = setInterval(() => {
      const url = targetUrl.includes('?') ? `${targetUrl}&_t=${Date.now()}` : `${targetUrl}?_t=${Date.now()}`;
      setImageUrl(url);
    }, every);
            ...
            <div className="flex flex-col border-b border-[var(--border-primary)] bg-black/60 relative z-10">
              ...
              <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-3">
                  ...
                  {onAddToGrid && (camera.feed_url || camera.stream_url) && (
                    <button onClick={() => onAddToGrid(camera)} className="p-1.5 rounded-sm bg-white/5 border border-white/10 hover:bg-[var(--gold-primary)]/20 hover:border-[var(--gold-primary)] transition-all" title="Add to CCTV wall">
                      <LayoutGrid className="w-3 h-3 text-[var(--text-secondary)] hover:text-[var(--gold-primary)]" />
                    </button>
                  )}
