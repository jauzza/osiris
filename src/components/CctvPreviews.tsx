...
function ImageMedia({ cam: camera, onReady, onFail }: MediaProps) {
  ...
  const every = refreshInterval(kind, camera);
  ...
  useEffect(() => {
    ...
    const stagger = every >= 3000 ? every + Math.random() * 2000 : Math.random() * 400;
...
function CctvPreviews({ mapRef, active, onOpen }: {
  ...
  const recompute = useCallback(() => {
    ...
    const viewingCosta = inCostaDelSolBbox(center.lat, center.lng);

    /* When over the Costa del Sol, rank-3 cams (Puerta Blanca / Molière /
       Alhaurín) win slots first; otherwise nearest the middle of the screen. */
    candidates.sort((a, b) => {
      if (viewingCosta) {
        const ra = cctvPriorityRank(a.cam);
        const rb = cctvPriorityRank(b.cam);
