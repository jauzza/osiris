...
describe('cityCanPanCam', () => {
  it('marks Ayto. Málaga TV-xx stills as city-steerable', () => {
    expect(cityCanPanCam({
      id: 'malaga-21608',
      source: 'Ayto. Málaga',
      feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fctraficomovilidad.malaga.eu%2Frecursos%2Fmovilidad%2Fcamaras_trafico%2FTV-84.jpg',
    })).toBe(true);
  });

  it('leaves DGT and tourism stills unmarked', () => {
    expect(cityCanPanCam({ id: 'dgt-175602', source: 'DGT' })).toBe(false);
    expect(cityCanPanCam({ id: 'meteo365-alhaurindelatorre', source: 'meteo365.es' })).toBe(false);
  });
});
