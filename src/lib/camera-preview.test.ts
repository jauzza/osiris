...
describe('refreshInterval', () => {
  ...
  it('refreshes Puerta Blanca snapshots every second', () => {
    expect(refreshInterval('jpg', { name: 'AVDA. MOLIERE', lat: 36.689, lng: -4.461 })).toBe(1000);
  });

  it('refreshes other Málaga snapshots every three seconds', () => {
    expect(refreshInterval('jpg', { name: 'CENTRO', lat: 36.72, lng: -4.42, city: 'Málaga' })).toBe(3000);
  });

  it('honours a forced grid-wall cadence', () => {
    expect(refreshInterval('jpg', { forceMs: 2000 })).toBe(2000);
  });
