...
describe('liveFeedAtSource', () => {
  ...
  it('offers the source page for a meteo365 camera we only get stills from', () => {
    const page = 'https://meteo365.es/livecams/malaga-misericordia.php';
    expect(liveFeedAtSource({ external_url: page, feed_url: JPG })).toBe(page);
  });

  it('does not claim live video exists behind other operators', () => {
    // Only SkylineWebcams and meteo365.es are known to keep a live stream
    // behind the still.
