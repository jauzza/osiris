...
/**
 * Ayto. Málaga TV-xx stills are the MOVIMA "cámaras móviles": the city can
 * pan them from Tabacalera. The public JPEG is read-only — this is a label,
 * not a control.
 */
export function cityCanPanCam(cam: { source?: string; id?: string; feed_url?: string }): boolean {
  if (cam.source !== 'Ayto. Málaga') return false;
  return /malaga-\d+/i.test(cam.id ?? '')
    || /camaras_trafico\/TV-\d+/i.test(cam.feed_url ?? '');
}
