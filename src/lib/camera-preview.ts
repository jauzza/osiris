import { cctvPriorityRank, type CctvLocation } from './cctv-priority';
 ...
 */
export interface RefreshContext extends CctvLocation {
  /** Grid wall: operator-selected cadence in ms. */
  forceMs?: number;
}

export function refreshInterval(kind: PreviewKind, ctx?: RefreshContext): number {
  if (ctx?.forceMs != null) return ctx.forceMs;
  if (kind === 'hls' || kind === 'mjpeg') return 0;

  const rank = ctx ? cctvPriorityRank(ctx) : 0;
