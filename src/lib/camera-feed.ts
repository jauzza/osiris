...
function isMeteo365Url(url: string): boolean {
  try {
    const host = new URL(url).hostname.toLowerCase();
    return host === 'meteo365.es' || host.endsWith('.meteo365.es');
  } catch {
    return false;
  }
}
...
export function liveFeedAtSource(camera: ResolvableCamera | null | undefined): string | null {
  ...
  if (isSkylineUrl(camera.external_url) || isMeteo365Url(camera.external_url)) {
    return camera.external_url;
  }
  return null;
}
