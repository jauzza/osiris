...
import { fetchMeteo365Cameras } from './meteo365';
import { fetchAndaluciaLiveCameras } from './andalucia-live';
...
const SKYLINE_SPAIN: CctvCamera[] = [
  ...
  { id: 'sky-es-marbella', lat: 36.5100, lng: -4.8820, name: 'Marbella - Puerto y Paseo', city: 'Marbella', country: 'Spain', feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fcdn.skylinewebcams.com%2Flive1140.jpg', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/marbella.html', source: 'SkylineWebcams' },
  { id: 'sky-es-marbella-marina', lat: 36.5065, lng: -4.8850, name: 'Marbella - Marina', city: 'Marbella', country: 'Spain', feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fcdn.skylinewebcams.com%2Flive2777.jpg', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/marbella-puerto-deportivo.html', source: 'SkylineWebcams' },
  { id: 'sky-es-benalmadena-stupa', lat: 36.5897, lng: -4.5164, name: 'Benalmádena - Estupa Budista', city: 'Benalmádena', country: 'Spain', feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fcdn.skylinewebcams.com%2Flive2424.jpg', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/benalmadena.html', source: 'SkylineWebcams' },
  { id: 'sky-es-montejaque', lat: 36.7358, lng: -5.2500, name: 'Montejaque - Plaza de la Constitución', city: 'Montejaque', country: 'Spain', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/montejaque.html', source: 'SkylineWebcams' },
  { id: 'sky-es-montejaque-pano', lat: 36.7365, lng: -5.2480, name: 'Montejaque - Panorama', city: 'Montejaque', country: 'Spain', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/panorama-montejaque.html', source: 'SkylineWebcams' },
  { id: 'sky-es-benalauria', lat: 36.5933, lng: -5.2614, name: 'Benalauría', city: 'Benalauría', country: 'Spain', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/benalauria.html', source: 'SkylineWebcams' },
  { id: 'sky-es-malaga-alameda', lat: 36.717781, lng: -4.421656, name: 'Málaga - Alameda Principal', city: 'Málaga', country: 'Spain', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/alameda-principal.html', source: 'SkylineWebcams' },
  { id: 'sky-es-malaga-costa', lat: 36.589196, lng: -4.529228, name: 'Costa del Sol - Benalmádena', city: 'Benalmádena', country: 'Spain', feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fcdn.skylinewebcams.com%2Flive5004.jpg', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/costa-del-sol.html', source: 'SkylineWebcams' },
  { id: 'sky-es-benalmadena-pano', lat: 36.5950, lng: -4.5150, name: 'Benalmádena - Panorama', city: 'Benalmádena', country: 'Spain', feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fcdn.skylinewebcams.com%2Flive4402.jpg', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/panorama-de-benalmadena.html', source: 'SkylineWebcams' },
  { id: 'sky-es-estepona', lat: 36.4250, lng: -5.1490, name: 'Estepona - Paseo Marítimo', city: 'Estepona', country: 'Spain', feed_url: '/api/cctv/proxy?url=https%3A%2F%2Fcdn.skylinewebcams.com%2Flive5557.jpg', external_url: 'https://www.skylinewebcams.com/en/webcam/espana/andalucia/malaga/estepona.html', source: 'SkylineWebcams' },
...
let malagaCache: { cameras: CctvCamera[], timestamp: number } | null = null;
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

const MALAGA_GEOJSON =
  'https://datosabiertos.malaga.eu/recursos/transporte/trafico/da_camarasTrafico-4326.geojson';

interface MalagaFeature {
...
function stripMalagaTvPrefix(name: string): string {
  return name.replace(/^TV-?\d+\s*-?\s*/i, '').trim() || name;
}

async function fetchMalagaCameras(): Promise<CctvCamera[]> {
  if (malagaCache && Date.now() - malagaCache.timestamp < CACHE_TTL_MS) {
    return malagaCache.cameras;
  }

  try {
    const response = await stealthFetch(MALAGA_GEOJSON, {
      signal: AbortSignal.timeout(15000),
    });
    if (!response.ok) return malagaCache?.cameras || [];
    ...
    for (const feature of data.features as MalagaFeature[]) {
      ...
      const rawName = props.NOMBRE || props.DESCRIPCION;
      if (!rawName) continue;

      const imageUrl = typeof props.URLIMAGEN === 'string' ? props.URLIMAGEN : '';
      const pageUrl = typeof props.URL === 'string' ? props.URL : undefined;

      cameras.push({
        id: `malaga-${props.ID ?? cameras.length}`,
        lat,
        lng,
        name: stripMalagaTvPrefix(rawName),
        city: 'Málaga',
        country: 'Spain',
        ...(imageUrl
          ? { feed_url: `/api/cctv/proxy?url=${encodeURIComponent(imageUrl)}` }
          : {}),
        ...(pageUrl ? { external_url: pageUrl } : {}),
        source: 'Ayto. Málaga',
      });
    }

    malagaCache = { cameras, timestamp: Date.now() };
    return cameras;
  } catch (error) {
    console.error('Failed to fetch Ayuntamiento de Málaga cameras:', error);
    return malagaCache?.cameras || [];
  }
}
...
async function fetchDGTCameras(): Promise<CctvCamera[]> {
  ...
  try {
    ...
    for (const cam of data.camaras) {
      ...
      cameras.push(labelAlhaurinCorridor({
        id: `dgt-${cam.id}`,
        lat,
        lng,
        name,
...
function nearCamera(a: CctvCamera, b: CctvCamera): boolean {
  return Math.abs(a.lat - b.lat) <= 0.001 && Math.abs(a.lng - b.lng) <= 0.001;
}

/** Prefer embeddable streams over link-out cards at the same pin. */
function cameraScore(cam: CctvCamera): number {
  if (cam.stream_url) return 3;
  if (cam.feed_url) return 2;
  if (cam.external_url) return 1;
  return 0;
}

function mergeCameras(base: CctvCamera[], extras: CctvCamera[]): CctvCamera[] {
  const merged = [...base];
  for (const extra of extras) {
    const idx = merged.findIndex(existing => nearCamera(existing, extra));
    if (idx < 0) {
      merged.push(extra);
      continue;
    }
    if (cameraScore(extra) > cameraScore(merged[idx])) {
      merged[idx] = extra;
    }
  }
  return merged;
...
export async function fetchSpainCameras(): Promise<CctvCamera[]> {
  const [dgtCameras, malagaCameras, meteo365Cameras, andaluciaCameras] = await Promise.all([
    fetchDGTCameras(),
    fetchMalagaCameras(),
    fetchMeteo365Cameras(),
    fetchAndaluciaLiveCameras(),
  ]);

  return mergeCameras(SPAIN_CAMERAS, [
    ...dgtCameras,
    ...malagaCameras,
    ...meteo365Cameras,
    ...andaluciaCameras,
