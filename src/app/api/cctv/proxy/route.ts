...
const ALLOWED_HOSTS = [
  ...
  'thb.gov.tw',
  'etraffic.dgt.es',
  'ctraficomovilidad.malaga.eu',
  'webcam.meteo365.es',
  'webcam2.meteo365.es',
  'meteo365.es',
  'cameras.measureeverything.io',
];
...
function refererFor(hostname: string): string | null {
  if (!sendsReferer(hostname)) return null;
  if (hostname === 'webcam.meteo365.es' || hostname === 'webcam2.meteo365.es') {
    return 'https://meteo365.es/';
  }
