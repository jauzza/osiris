...
import { Layers, BarChart3, Newspaper, Search, X, Globe, MapPinned, Route, Radar, Satellite, Moon, ExternalLink, AlertTriangle, Activity, Database, Wifi, Play, Network, Crosshair, Bluetooth, Pentagon, Radio, PenLine, LayoutGrid } from 'lucide-react';
...
const CctvGridWall = dynamic(() => import('@/components/CctvGridWall'));
...
export default function Dashboard() {
  ...
  const [showCctvGrid, setShowCctvGrid] = useState(false);
  const [cctvGridSlotIds, setCctvGridSlotIds] = useState<string[]>(() => loadCctvGridState().slotIds);
  const [cctvGridRefreshMs, setCctvGridRefreshMs] = useState(() => loadCctvGridState().refreshMs);
  const [cctvGridPendingAdd, setCctvGridPendingAdd] = useState<any>(null);
  const [cctvGridPickMode, setCctvGridPickMode] = useState(false);
  const appliedDefaultGrid = useRef(loadCctvGridState().slotIds.some(Boolean));
  ...
  // Pinch / ctrl-wheel belongs to the map, not Chrome or Safari page zoom.
  useEffect(() => {
    ...
    const detach = attachBrowserZoomGuard(document);
    return () => {
  ...
  const handleEntityClick = useCallback((entity: any) => {
    if (entity?.type === 'cctv') {
      if (cctvGridPickMode) {
        if (!previewMedia(entity)) return;
        const { slots, complete } = addCameraToGridSlots(cctvGridSlotIds, String(entity.id));
        setCctvGridSlotIds(slots);
        setShowCctvGrid(true);
        if (complete) setCctvGridPickMode(false);
        return;
      }
  ...
  }, [cctvGridPickMode, cctvGridSlotIds]);

  const handleCctvGridPickMode = useCallback((on: boolean) => {
    setCctvGridPickMode(on);
    if (on) {
      setCctvGridSlotIds(normalizeGridSlots([]));
      setShowCctvGrid(true);
    }
  }, []);

  const handleAddCameraToGrid = useCallback((camera: any) => {
    setShowCctvGrid(true);
    setCctvGridPendingAdd(camera);
  }, []);

  useEffect(() => {
    saveCctvGridState({ slotIds: cctvGridSlotIds, refreshMs: cctvGridRefreshMs });
  }, [cctvGridSlotIds, cctvGridRefreshMs]);

  useEffect(() => {
    if (appliedDefaultGrid.current || cctvGridPickMode) return;
    if (cctvGridSlotIds.some(Boolean)) {
      appliedDefaultGrid.current = true;
      return;
    }
    const cameras = dataRef.current.cameras as Array<{
      id: string; name: string; lat: number; lng: number;
      feed_url?: string; stream_url?: string;
    }> | undefined;
    if (!cameras?.length) return;
    const preset = pickPuertaBlancaGrid(cameras, 4);
    if (!preset.length) return;
    appliedDefaultGrid.current = true;
    setCctvGridSlotIds(normalizeGridSlots(preset.map(c => c.id)));
  }, [dataVersion, cctvGridSlotIds, cctvGridPickMode]);
  ...
  return (
    <main data-osiris-dashboard className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden touch-none overscroll-none">
      ...
      <ErrorBoundary name="Map">
        <OsirisMap 
          ...
          cctvPickMode={cctvGridPickMode}
        />
      </ErrorBoundary>

      {cctvGridPickMode && (
        <div className="pointer-events-auto absolute left-1/2 top-16 z-[450] -translate-x-1/2 md:top-20">
          <div className="flex items-center gap-3 rounded border border-[var(--map-cctv)]/50 bg-black/85 px-3 py-2 shadow-lg backdrop-blur-md">
            <Crosshair className="h-4 w-4 shrink-0 animate-pulse text-[var(--map-cctv)]" />
            <div>
              <p className="font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-[var(--map-cctv)]">Pick mode</p>
              <p className="font-mono text-[8px] tracking-wider text-white/50">
                Click {4 - cctvGridSlotIds.filter(Boolean).length} camera{4 - cctvGridSlotIds.filter(Boolean).length === 1 ? '' : 's'} on the map · {cctvGridSlotIds.filter(Boolean).length}/4
              </p>
            </div>
            <button
              type="button"
              onClick={() => setCctvGridPickMode(false)}
              className="rounded border border-white/15 px-2 py-1 font-mono text-[8px] tracking-wider text-white/60 hover:text-white"
            >
              Cancel
            </button>
          </div>
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        ...
        {activeLayers.cctv && (
          <div className="relative group">
            <button
              onClick={() => setShowCctvGrid(v => !v)}
              className={`relative w-8 h-8 rounded-full flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-white/50 ${showCctvGrid ? 'bg-[var(--map-cctv)]/20' : 'hover:bg-white/10'}`}
              title="CCTV wall — 4-up grid with fast refresh"
              aria-label="CCTV wall"
              aria-expanded={showCctvGrid}
            >
              <LayoutGrid className={`w-4 h-4 ${showCctvGrid ? 'text-[var(--map-cctv)]' : 'text-white/60'}`} />
            </button>
            <span className="absolute right-11 top-1/2 -translate-y-1/2 px-2 py-1 text-[9px] font-mono tracking-wider text-white/80 bg-black/80 backdrop-blur-sm rounded whitespace-nowrap opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity pointer-events-none">CCTV WALL</span>
          </div>
        )}
      ...
      <CameraViewer
        ...
        onLocate={(lat, lng) => setFlyToLocation({ lat, lng, zoom: 15, ts: Date.now() })}
        onAddToGrid={handleAddCameraToGrid}
      />

      <CctvGridWall
        open={showCctvGrid}
        cameras={data.cameras ?? []}
        slotIds={cctvGridSlotIds}
        onSlotIdsChange={setCctvGridSlotIds}
        refreshMs={cctvGridRefreshMs}
        onRefreshMsChange={setCctvGridRefreshMs}
        onClose={() => { setShowCctvGrid(false); setCctvGridPickMode(false); }}
        onLocate={(lat, lng) => setFlyToLocation({ lat, lng, zoom: 15, ts: Date.now() })}
        pendingAdd={cctvGridPendingAdd}
        onPendingAddHandled={() => setCctvGridPendingAdd(null)}
        pickMode={cctvGridPickMode}
        pickCount={cctvGridSlotIds.filter(Boolean).length}
        onPickModeChange={handleCctvGridPickMode}
      />
